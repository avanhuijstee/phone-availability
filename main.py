...
def _send_push_sync(sub: dict, data: str) -> int:
    ...
    encoded = wp.encode(data.encode())
    token = _make_vapid_jwt(sub["endpoint"])
    headers = dict(encoded.headers)
    headers["Authorization"] = f"vapid t={token},k={VAPID_PUBLIC_KEY}"
    headers["TTL"] = "60"
    headers["Urgency"] = "high"
    resp = _requests.post(sub["endpoint"], data=encoded.body, headers=headers, timeout=10)
